...
function profileLookup() {
	...
	$knownReportKeysLookup = mysql_query("select REPORT_KEY from radiobox_reportRecord group by REPORT_KEY");
	...
	while(list($report_id, $report_date) = each($report_ids)) {
		$queryString = "select REPORT_KEY,REPORT_VALUE from radiobox_reportRecord where REPORT_ID='" . $report_id . "'";
