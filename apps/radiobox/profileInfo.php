...
if (array_key_exists('appName', $_GET)) {
  ...
  $queryString = "INSERT INTO radiobox_profileReport (IP_ADDR, REPORT_DATE) VALUES (\"". $_SERVER["REMOTE_ADDR"] . "\",\"" . $report_date .  "\")";
  if ($debug) {
  ...
  // Date, 
  	if (array_key_exists($key, $appcastKeys) && $appcastKeys[$key] == 1) {
  		...
  		$queryString = "INSERT INTO radiobox_reportRecord (REPORT_KEY, REPORT_VALUE, REPORT_ID) VALUES (\"" . $key . "\",\"" . $value . "\",\"" . $record_id . "\")";
  		if ($debug) {
  			print "$key: $value<br />\n";
  			print "$queryString<br />";
  		}
