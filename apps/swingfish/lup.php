...
function profileLookup() {
	...
	$queryString = "select REPORT_ID,REPORT_DATE from swingfish_profileReport where REPORT_DATE >= '" . $start_date . "' and REPORT_DATE <= '" . $end_date . " ORDER BY REPORT_DATE'";
	...
	$knownReportKeysLookup = mysql_query("select REPORT_KEY from swingfish_reportRecord group by REPORT_KEY");
	...
	while(list($report_id, $report_date) = each($report_ids)) {
		$queryString = "select REPORT_KEY,REPORT_VALUE from swingfish_reportRecord where REPORT_ID='" . $report_id . "'";
